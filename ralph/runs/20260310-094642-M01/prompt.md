...
- Important blocker: `npm install` never completed in this sandbox and produced no `node_modules`, so `npm run lint`, `npm run build`, and `npm run test` fail because `tsc` and `vitest` are unavailable. Resolve environment/package installation before starting the next milestone.
